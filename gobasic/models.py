...
class Hotel(models.Model):
    ...
    net_cp = models.PositiveIntegerField(
        ...
        verbose_name="CP",
        default=0,
        help_text="Per Day for 2pax",
    )
    net_map = models.PositiveIntegerField(
        validators=[MaxValueValidator(1000000), MinValueValidator(1)],
        verbose_name="MAP",
        default=0,
        help_text="Per Day for 2pax",
    )
    net_ap = models.PositiveIntegerField(
        validators=[MaxValueValidator(1000000), MinValueValidator(1)],
        verbose_name="AP",
        default=0,
        help_text="Per Day for 2pax",
    )
    net_cp_kid = models.PositiveIntegerField(
        validators=[MaxValueValidator(1000000), MinValueValidator(1)],
        verbose_name="CP Kid",
        default=0,
        help_text="Per Day for 1pax",
    )
    net_map_kid = models.PositiveIntegerField(
        validators=[MaxValueValidator(1000000), MinValueValidator(1)],
        verbose_name="MAP Kid",
        default=0,
        help_text="Per Day for 1pax",
    )
    net_ap_kid = models.PositiveIntegerField(
        validators=[MaxValueValidator(1000000), MinValueValidator(1)],
        verbose_name="AP Kid",
        default=0,
        help_text="Per Day for 1pax",
    )
    hotel_email = models.CharField(max_length=50, blank=True, null=True)
...
class Activity(models.Model):
    ...
    net_cost = models.PositiveIntegerField(
        validators=[MaxValueValidator(100000), MinValueValidator(1)], default=0
    )
...
class Customer(models.Model):
    ...
    mobile = models.CharField(
        max_length=12, unique=True, help_text="<em>10 digits</em>"
    )
...
class Transfer(models.Model):
    ...
    customer_transfer = models.OneToOneField(
        Customer,
        on_delete=models.PROTECT,
        blank=True,
        verbose_name="Customer->Transfer",
    )
...
class Trip(models.Model):
    ...
    start_date = models.DateTimeField(
        default=timezone.now, help_text="yyyy-mm-dd,hh--mm"
    )
    end_date = models.DateTimeField(default=timezone.now)
    profit_percentage = models.PositiveSmallIntegerField(
        validators=[MaxValueValidator(25), MinValueValidator(7)],
        default=11,
        verbose_name="Profit %",
        help_text="Profit %",
    )
    ...
    hotel_pb = models.ForeignKey(
        Hotel,
        verbose_name="Hotel Port Blair",
        related_name="pb_hotel_set",
        limit_choices_to={"location": "Port Blair"},
        on_delete=models.PROTECT,
        blank=True,
        null=True,
    )
    plan_pb = models.CharField(
        max_length=11, verbose_name="Meal Plan", default="CP", choices=meal_plan
    )
    pb_rooms = models.PositiveSmallIntegerField(
        validators=[MaxValueValidator(30), MinValueValidator(0)],
        default=0,
        verbose_name="Port Blair Rooms",
        help_text="Number of Rooms",
    )
    pb_nights = models.PositiveSmallIntegerField(
        validators=[MaxValueValidator(10), MinValueValidator(0)],
        default=0,
        verbose_name="Port Blair Nights",
        help_text="Port Blair Nights",
    )
    pb_add_on = models.PositiveBigIntegerField(
        validators=[MaxValueValidator(100000), MinValueValidator(0)],
        default=0,
        verbose_name="Port Blair Add-On",
        help_text="Port Blair Hotel Add On",
    )

    # destination 2 hotel related fields below
    hotel_hv = models.ForeignKey(
        Hotel,
        verbose_name="Hotel Havelock",
        related_name="hv_hotel_set",
        limit_choices_to={"location": "Havelock Island"},
        on_delete=models.PROTECT,
        blank=True,
        null=True,
    )
    plan_hv = models.CharField(
        max_length=11, verbose_name="Meal Plan", default="CP", choices=meal_plan
    )
    hv_rooms = models.PositiveSmallIntegerField(
        validators=[MaxValueValidator(30), MinValueValidator(0)],
        default=0,
        verbose_name="Havelock Rooms",
        help_text="Number of Rooms",
    )
    hv_nights = models.PositiveSmallIntegerField(
        validators=[MaxValueValidator(10), MinValueValidator(0)],
        default=0,
        verbose_name="Havelock Nights",
    )
    hv_add_on = models.PositiveBigIntegerField(
        validators=[MaxValueValidator(100000), MinValueValidator(0)],
        default=0,
        verbose_name="Havelock Hotel Add-On",
        help_text="Havelock Hotel Add On",
    )

    # destination 3 hotel related fields below
    hotel_nl = models.ForeignKey(
        Hotel,
        verbose_name="Hotel Neil",
        related_name="nl_hotel_set",
        limit_choices_to={"location": "Neil Island"},
        on_delete=models.PROTECT,
        blank=True,
        null=True,
    )
    plan_nl = models.CharField(
        max_length=11, verbose_name="Meal Plan", default="CP", choices=meal_plan
    )
    nl_rooms = models.PositiveSmallIntegerField(
        validators=[MaxValueValidator(30), MinValueValidator(0)],
        default=0,
        verbose_name="Neil Rooms",
        help_text="Number of Rooms",
    )
    nl_nights = models.PositiveSmallIntegerField(
        validators=[MaxValueValidator(10), MinValueValidator(0)],
        default=0,
        verbose_name="Neil Nights",
    )
    nl_add_on = models.PositiveBigIntegerField(
        validators=[MaxValueValidator(100000), MinValueValidator(0)],
        default=0,
        verbose_name="Neil Hotel Add-On",
        help_text="Neil Hotel Add On",
    )
    ...
    activities = models.ManyToManyField(
        ...
        blank=True,
        help_text="select multiple, note locations",
    )
    transfers = models.ForeignKey(
        Transfer, blank=True, null=True, on_delete=models.PROTECT
    )
